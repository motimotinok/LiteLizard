...
- タスク中または完了時に、再利用可能な学び・作業ルール・プロンプト改善点が見つかった場合は、`CLAUDE.md` または該当する `.claude/skills/*/SKILL.md` に最小限の追記を行ってよい
- 自己改善の追記は、安定した運用ルール・再発防止・検証手順の改善に絞る。一時的な作業メモ、今回限りの判断、未確定の実験フローは `CLAUDE.md` に固定しない
