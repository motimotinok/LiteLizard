...
3. **Step 3: code-reviewer（レビュー）** — Full / Light チェーン
   ...
   - プロンプトにはレビュー対象ファイルリストを必ず含める（code-reviewer はそのファイルのみ `git diff -- <files>` でレビューする）
   - 全タスクの完了通知を待つ
...
5. **次の Wave に進む**
   - Wave 間に依存がある場合、プランの「Wave 間依存」セクションに記載された依存理由を確認し、前 Wave の該当成果物（型定義・エクスポート・ストアスライス等）が正しく生成されていることを検証してから次 Wave を開始する
...
**メモリ書き込みの制約**: 並列実行時、複数のサブエージェントが同じエージェントメモリディレクトリ（`.claude/agent-memory/<agent>/`）に同時書き込みするとファイル競合が発生する。並列起動するサブエージェントのプロンプトには**「メモリの更新は行わないこと」**を明記する。
