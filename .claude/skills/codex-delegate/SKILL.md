...
2. 実行前に確認する：
   - 対象ディレクトリ（デフォルト: `/Users/jane/liteLizard`）
   - Codex に渡すプロンプト（日本語可）
3. 承認を得たら Bash で実行する：
   ```bash
   ~/.local/bin/codex-delegate gen /Users/jane/liteLizard "（Codex へ渡すプロンプト）"
   ```
...
1. Bash で実行する：
   ```bash
   ~/.local/bin/codex-delegate review /Users/jane/liteLizard
   ```
