...
description: "LiteLizard の GitHub Issue backlog を整理・優先度確認するスキル。ユーザーが「次に何をやるべき」「Issue を整理して」「優先度を見て」「backlog を見て」「公開前に残っているものを見て」などと依頼したときに使う。`NOW.md`、open Issues、`docs/tickets/`、`CHANGELOG.md` を読み、次の 1 件または小さな候補群を提示する。原則 non-mutating で、Issue 作成・コメント・ラベル変更・Ralph チケット化は明示依頼がある時だけ行う。"
---
...
- `NOW.md`、open Issues、`docs/tickets/`、`CHANGELOG.md` を確認する
- 次の 1 件、または必要最小限の候補群に絞って提示する
- GitHub Issue、Ralph チケット、CHANGELOG、仕様文書の役割を混ぜない
...
- `docs/tickets/`: すでに実行キューに切り出された作業
...
- すでに `docs/tickets/` に切り出された作業は、重複して Issue から再提案しない
- Issue が実装済みに見える場合は、コード・CHANGELOG・関連チケットで確認してから「close 確認候補」として出す
...
- **整理結果**: 重複、実装済み疑い、blocked、Ralph チケット化候補
