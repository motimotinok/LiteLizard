...
MVP 公開版は、未署名の macOS `.dmg` として GitHub Releases から配布する想定です。現時点では Apple Silicon Mac 向けの `LiteLizard-<version>-arm64.dmg` を動作確認対象にしています。Developer ID 署名、notarization、自動更新は MVP 後の対応です。

1. [`Releases`](https://github.com/motimotinok/LiteLizard/releases) から最新の `LiteLizard-<version>-arm64.dmg` をダウンロードします。
...
- 公開用の未署名 `.dmg` 生成: `pnpm --filter @litelizard/desktop package:mac:dmg`
  - 出力先: `apps/desktop/release/LiteLizard-<version>-<arch>.dmg`（例: `LiteLizard-0.1.0-arm64.dmg`）
