...
MVP 公開版は、未署名の macOS `.dmg` として GitHub Releases から配布する想定です。現時点では Apple Silicon Mac 向けの `LiteLizard-latest-arm64.dmg`（常に最新版を指す固定ファイル名）を動作確認対象にしています。Developer ID 署名、notarization、自動更新は MVP 後の対応です。
