...
> **退役済み**: この WBS は履歴参照用。現役の実装キューは `docs/tickets/`、現在地は `NOW.md`、backlog は GitHub Issues を参照する。今後このファイルへ新規タスク追加・完了更新は行わない。
...
| L-09 | 分析コンテキストポリシー切替（文書全体/章内・上限なし/直近N） | L-04 | P2 | M | ✅ | Codex | dev |
...
- **完了**: 2026-05-06。設定保存、SettingsScreen UI、main 側 context 生成、回帰テストを実装済み。詳細は `CHANGELOG.md` [2026/05/06] L-09 を参照
...
| T-04 | 章削除・段落統合のエッジケーステスト | R-01, R-02 ✅ | P1 | M | ✅ | Claude | ralphloop |
| T-05 | 解析コンテキストポリシーの回帰テスト | L-09 ✅ | P2 | S | ✅ | Codex | dev |
...
| L: ローカル分析基盤 | 9 | 9 | 0 | 0 | 7 | 2 | 0 |
| A: クラウドログイン（将来） | 5 | 0 | 5 | 0 | 0 | 0 | 5 |
| R: Renderer/UI | 22 | 16 | 6 | 0 | 9 | 12 | 1 |
| T: テスト | 6 | 6 | 0 | 0 | 5 | 1 | 0 |
| **合計** | **60** | **49** | **11** | **13** | **26** | **15** | **6** |
