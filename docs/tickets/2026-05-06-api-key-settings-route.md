...
- [ ] API キー未設定時に、解析 UI から設定画面への導線がある
- [ ] SettingsScreen で OpenAI / Anthropic API キー、既定 provider / model、ローカル LLM 設定を保存できる
- [ ] 保存後、解析実行可否の判定が設定内容を反映する
- [ ] fresh 起動相当の状態でも、ユーザーが詰まらず解析設定を完了できる
- [ ] 既存の解析実行、Reading Agent、ローカル LLM 設定が壊れていない
