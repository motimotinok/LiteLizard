...
- [ ] ルートに `PRIVACY.md` が追加されている
- [ ] 原稿本文と分析結果のローカル保存場所が説明されている
- [ ] API キー保存の扱いが説明されている
- [ ] 外部 API 利用時に送信されるデータの種類が説明されている
- [ ] ローカル LLM 利用時の送信先がユーザー設定 endpoint であることが説明されている
- [ ] LiteLizard 自体がサーバーへ本文を送らない範囲が、現行実装と矛盾なく書かれている
- [ ] README から `PRIVACY.md` を確認できる

## 検証方法

- [ ] `PRIVACY.md` の記述が現行の保存・分析・設定実装と矛盾していないか確認する
