...
改訂: 2026-06-23 legacy API 削除により将来のクラウド認証を新規設計対象として明記

> **S-09 方針**: 分析機能はログイン不要で利用可能（外部 API キーまたはローカル LLM）。クラウドサーバーログイン（OAuth）は将来の拡張パスとして残す。
>
> **現在の実装状態**: 認証サーバーとセッションAPIは存在しない。旧Fastify APIは削除済みであり、将来クラウド機能へ着手するときはOAuthプロバイダー、トークン契約、失効・更新、運用境界を含めて新規設計する。
...
削除したlegacy APIのemail-link認証やJWT実装は再利用しない。
