...
改訂: 2026-07-01 Reading Agentごとの構造化タグ定義を追加、分析結果契約を response + 任意 tags へ移行、分析履歴へ最小来歴と本文fingerprintを保存、実行前確認内容と省略設定を追加、現行公開版の実行対象は段落のみと明記 / 2026-06-30 contextPolicy を Reading Agent 所有の設定へ移行 / provider 既定モデルを候補選択化 / OpenAI prompt cache prefix を安定化
...
現行公開版で AI に読ませて保存できる実行対象は段落のみであり、章・文書全体・一文を独立した `targetScope` として実行、保存、履歴表示する経路は持たない。章サマリーは段落分析履歴の集計表示であり、章本文全体を 1 つの対象として provider に渡す AI 章分析ではない。詳細は [`analysis-scope-boundaries.md`](analysis-scope-boundaries.md) を参照。

### 1.2 操作パターン

| 操作 | 送信範囲 | 備考 |
|------|---------|------|
| 章内段落の一括分析 | 章内の全段落 | 章そのものではなく、章内の各段落を順に分析する |
| 特定段落の再分析 | 対象段落1つ | ユーザープロンプト入力付き（UI は都度検討） |
| ドキュメント内段落の一括分析 | 全段落 | 文書そのものではなく、文書内の各段落を順に分析する。トークン消費確認ダイアログを表示 |
...
- 各段落の詳細インスペクターに「履歴」入口を表示する
- デフォルトは最新の互換パターン（`patterns` 配列のうち本文 fingerprint または旧 `sourceText` が一致する末尾）を表示する
- 「履歴」を開いたときだけ、前後ボタン、現在位置、保存日時、実行時の Reading Agent 名を表示する
- `<` 相当の操作で1つ前の互換パターン、`>` 相当の操作で1つ後の互換パターンに切り替える
- 最古 / 最新の端では、それ以上進めないボタンを disabled にする
- `provenance.agentName` を持たない旧履歴は「旧形式の履歴」として扱い、表示を壊さない
