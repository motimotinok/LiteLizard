...
OpenAI / Anthropic / local-llm の各 provider には、agent の `model` があればそれを渡し、`null` の場合は選択 provider の既定 model を渡す。`temperature` は Reading Agent の設定項目にせず、provider へも送らない。読者の揺らぎや慎重さは `systemPrompt` で表現する。
...
AgentsScreen は `name` / `role` / `systemPrompt` / `contextPolicy` / `tagDefinitions` / `model` を controlled draft として扱い、保存、新規作成、削除、複製、リセット、テンプレート追加、dirty 表示、validation を提供する。新規作成Agentの参照範囲は `whole-document`、タグ定義は空を既定にする。削除は確認を出し、0件状態ではテンプレート追加導線を表示する。
...
- agent の `model` override と `contextPolicy` が通常分析、1段落再分析、dry-run、実行前見積もりに使われ、`temperature` は provider へ送られない。
