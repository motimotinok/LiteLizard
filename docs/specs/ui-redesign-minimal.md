...
- 段落番号: 漢数字 `一・二・…・九十九`（100 以上は半角フォールバック）
- 段落ホバー: 背景 `rgba(28,40,80,0.025)` + 左 1.5px 藍縦線
- ドラッグハンドル: ホバー時のみ表示
...
- Recent files の永続化
- 検索画面の中身
- 段落 ↔ パネル連動の fade highlight アニメーション（仕様策定済み、実装は別タスク）
