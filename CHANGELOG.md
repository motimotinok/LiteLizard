...
READMEと公開記事下書きにある問題意識を基に、文章の読まれ方を段落単位で観測する特徴、Reading Agentを使う3ステップ、書く場所と見直す場所を一つにした開発背景をランディングページへ追加した。冒頭・ヘッダー・末尾の3か所から最新版DMGへ進める導線を設け、デスクトップとスマートフォン向けのレスポンシブ表示を整えた。検証: landingのVite build、デスクトップ/モバイルのブラウザ表示、ページ内ナビゲーション、横方向のはみ出し、console warning/errorなしを確認。
