...
[2026/07/01]#115 抽象化した読書環境プレビューを当面非採用として記録
原稿を複数の読書環境プリセットへ置き直すプレビュー機能は、#114 の恒常テーマ基盤に依存する発展機能であり、現時点では実装しない判断を `docs/specs/reading-environment-preview.md` と `docs/decisions.md` に記録した。将来再検討する場合の境界として、特定サービス模倣なし、一時表示のみ、本文・分析・恒常テーマへ副作用なしを明示した。
