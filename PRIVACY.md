...
- LiteLizard はユーザーが指定したフォルダ以外には原稿を書き込みません
...
- LiteLizard は API キーを LiteLizard 自体のサーバーへ送信しません。設定された外部 provider への HTTP リクエストヘッダや、ユーザーが指定した Local LLM endpoint への接続でのみ使用されます

## 外部送信されるデータ

LiteLizard は分析機能を実行したときに限り、設定された外部 provider または Local LLM endpoint に対して、本文とコンテキスト段落を送信します。送信先と送信内容は次の通りです。
...
- 送信先: ユーザーが設定画面で指定した endpoint URL（例: Ollama 互換の `http://localhost:11434`）の `/api/generate` および接続確認用の `/api/tags`
